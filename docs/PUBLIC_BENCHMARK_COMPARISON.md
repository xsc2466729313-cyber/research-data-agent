...
目前已完成三层可复现的公开数据评测：检索、实体匹配和数据清洗。结果不能合成一个总分，因为三层解决的是不同问题，分数的分母也不同。

最重要的发现有两个：

1. 当前项目的离线哈希检索低于 BM25；这说明它还没有获得可靠的语义检索增益。
2. 在实体匹配的学术记录数据上，保守规则有较高 F1；但在商品记录数据上召回率明显不足。它不能直接迁移为患者或样本身份合并能力。
...
| 字段对齐 | Valentine | 未运行 | 官方完整实验数据需单独获取；还需要为项目 schema mapper 提供通用表格适配器 |
