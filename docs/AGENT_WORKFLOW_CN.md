...
对应的内核实验室红色讲解截图见 [前端与内核截图索引](FRONTEND_SCREENSHOT_INDEX.md) 的 K07；完整流程图和离线 PNG 也在同一索引的 D 组。
...
- **纸张形状**：输入或输出，例如研究问题和科研数据交付包。
- **圆角框**：可以理解任务、选择下一步并根据反馈继续行动的 Agent。
- **直角框**：按固定程序执行的规则模块，例如真实取数、字段标准化和身份核对。
- **菱形**：判断节点，例如关键字段是否齐全、是否通过质量与医学安全门。
