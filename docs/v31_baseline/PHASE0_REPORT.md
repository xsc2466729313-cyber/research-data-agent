...
| 项 | 值 |
|---|---|
| 项目名称 | 肿瘤精准治疗科研数据智能整合系统（cancer-precision-data-agent） |
...
```text
git diff --name-only -- configs/canonical_schema.yaml configs/medical_rules.yaml docs/06_评测指标与SDTI.md
```
...
| `docs/06_评测指标与SDTI.md` | 无 diff | `D285926B0D80384F6145B9EBA54FD5D9C6609C8A69F2302440671D5B685D592D` |
...
完整清单见 `docs/v31_baseline/API_SNAPSHOT.md`。
...
完整契约见 `docs/v31_baseline/OUTPUT_CONTRACT.md`。
...
运行方式与文件清单见 `docs/v31_baseline/REGRESSION.md`。
...
按 `IMPLEMENTATION_PLAN_V31.md`，第一刀只做旁路对话闸门，不自动取数，不创建完整 Planner（Planner 门面可作为 Phase 1 第二刀）。
