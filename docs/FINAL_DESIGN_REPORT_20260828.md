...
当前系统已经形成“问题理解 → 真实数据获取 → 标准化与患者/样本关联 → 医学安全门控 → 两轮闭环 → 可追溯导出”的可运行链路。Qwen-plus 是默认规划模型，DeepSeek、GLM 和 OpenAI-compatible 模型通过同一会话接口接入评测或受控运行，但没有把未真实调用的模型标成高分。
...
| DeepSeek Chat / Reasoner | 可选兼容模型或独立评委 | 已有 provider/session 适配；需独立 Key 才运行 |
| GLM / OpenAI-compatible | 外部评测或自定义端点 | 通过兼容接口配置；未连接不计分 |
