...
Qwen-plus 已完成本机网络、鉴权、模型可用性和 Function Calling 探测。DeepSeek、GLM 本轮没有独立凭据，因此没有生成同条件横向排名。正式乳腺癌 Gold Set 尚未冻结，故检索 F1、忠实度、可追溯率、错误检测 F1、修复正确率和 SDTI 仍为 `NOT_EVALUATED`。
...
| 字段对齐 | **V2 宏平均 F1：0.8451** | Valentine 10 个公开任务 | 通用字段匹配，不等同医学字段 Gold Set |
| 实体关联 | **V2 宏平均 F1：0.7408** | DeepMatcher 5 个公开任务 | 不等同患者身份匹配成绩 |
| 模型接入 | **Qwen-plus 结构化 Agent 探测通过** | 本机真实连接、鉴权与 Function Calling | 证明可用，不构成模型质量排名 |
| 闭环执行 | **固定两轮、输入输出可审计** | 真实模式 smoke 与计划模式诊断 | 证明闭环机制可运行，不构成 SDTI |
...
逐数据集看，**本项目 BGE 的 nDCG@10 均高于项目 BM25 调参版**。因此 BGE 是当前公开检索评测中的推荐方案；BM25 保留为依赖少、成本低的生产回退。未达到主结果标准的融合方案不在本节展开，完整原始对照仍保存在分层评测报告中。
...
本项目默认的字段对齐方案为 **value-profile V2**。在 10 个 Valentine 公开任务中，其宏平均精确率为 **0.9286**、召回率为 **0.7864**、F1 为 **0.8451**。该结果用于说明通用字段对齐能力；乳腺癌关键字段仍需经过 Canonical Schema、原始值保留和医学规则复核。

### 6.3 实体关联

本项目默认的实体关联方案为 **learned entity rule V2**。在 5 个 DeepMatcher 公开任务中，其宏平均精确率为 **0.7259**、召回率为 **0.7668**、F1 为 **0.7408**。这是通用实体关联证据，不是患者身份自动合并许可；患者/样本低置信度关联仍进入 `REVIEW` 或 `unresolved`。
