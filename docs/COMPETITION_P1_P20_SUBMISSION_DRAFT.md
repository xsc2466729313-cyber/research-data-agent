...
以下是本项目在相同公开任务上重跑的“对照组/实验组”，不是外部模型排名。为避免把原始医学问题和通用能力题混在一起，原问题保留为 `RQ-01`，公开拆分题编号为 `PB-01`—`PB-04`；完整解释见 `docs/PUBLIC_COMPARISON_GUIDE_20260902.md`。

| 能力 | 对照组（本项目基线） | 实验组（本项目方案） | 结果 |
|---|---|---|---:|
| `PB-01` 科学检索 | BGE-small-en-v1.5 单路 | **BM25 + BGE + CrossEncoder** | nDCG@10 `0.3818` vs `0.3880`；较旧融合 `0.3791` 提升 `0.0027`，仍低于公开对照 `0.0061` |
| `PB-02` 字段对齐 | Valentine COMA | **Schema Matcher V3** | **F1 `0.7994` vs `0.7670`，本项目高 `0.0324`** |
| `PB-03` 实体关联 | RecordLinkage | **自适应实体融合** | **F1 `0.7449` vs `0.7440`，本项目高 `0.0009`** |
| `PB-04` 错误检测 | Raha PVD+RVD | **格式与占位符融合** | Cell F1 `0.5726` vs `0.8159`，公开对照高 `0.2433` |

这些指标分别来自 BEIR、Valentine、DeepMatcher 和 Raha/HoloClean，证明的是模块通用能力，不是乳腺癌临床效果。当前真实结论是字段匹配和实体匹配宏平均领先，检索接近但略低，错误检测落后；不能为了形成“全项领先”而删掉失败任务或更换指标。
