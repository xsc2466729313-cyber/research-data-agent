...
> 发布口径（v2.2.0）：当前唯一对外引用的完整链路观察值是候选卷严格千问在线 `SDTI 98.1118`；同卷确定性消融为 `100.00`。两者均为 `publish_allowed=false` 的工程观察，不是封存 `frozen_test` 正式成绩。正文后半部分保留带日期的历史运行值，用于解释改进过程，不覆盖本发布口径。
...
![图 1 当前科研规划工作台（带红框、箭头和中文说明）](images/frontend-home-annotated-20260906.png)

*图 1 为 2026-09-06 重新采集的当前入口。左侧五阶段导航负责推进研究，中央区域提供跨癌种示例问题与一句话输入，右侧标签页承载研究依据、研究方案、数据准备和覆盖矩阵。红框与箭头用于说明页面分区，不代表某次运行已经完成取数。*
...
当前根地址已恢复为“新研究 / 提出方向”规划工作台。以下两张图均为 2026-09-06 重新采集的页面，并保留红框、箭头和中文说明；后续图 7—图 14 仍是 2026-08-31 代表任务的结果审查证据，表示历史运行中的数据表、质量门和来源血缘，不应与当前入口截图混用。

![图 2 当前科研规划工作台桌面布局](images/frontend-home-annotated-20260906.png)

*图 2 展示五阶段导航、示例问题卡、研究问题输入区和右侧四类审查标签。*

![图 3 当前科研规划工作台移动端布局](images/frontend-home-mobile-annotated-20260906.png)

*图 3 展示窄屏下示例卡片、输入区和审查标签的顺序重排；页面仍保留“发送并开始研究”入口。*
