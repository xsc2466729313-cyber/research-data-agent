# 面向多癌种精准治疗的科研数据智能整合系统：设计与验证

在线演示入口：[打开科研规划工作台](https://cancer-precision-data-agent.onrender.com/)。该地址为当前 Render 公网实例，可直接分享访问；免费实例休眠后首次打开可能需要等待几十秒。

> 发布口径（v2.2.1）：当前唯一对外引用的完整链路观察值是候选卷严格千问在线 `SDTI 98.1118`；同卷确定性消融为 `100.00`。两者均为 `publish_allowed=false` 的工程观察，不是封存 `frozen_test` 正式成绩。v2.2.1 仅补齐截图、流程图和阅读包，后端运行时仍为 `2.2.0-qwen-agent`；正文后半部分保留带日期的历史运行值，用于解释改进过程，不覆盖本发布口径。

> 参赛方向：方向 1“科学数据整合与影响力分析应用”·A“科学数据查找、解析与整合”  
> 项目名称：科研数据智能体<br>
> 数据场景：多癌种精准治疗科研数据的查找、解析、整合、核对与复用；乳腺癌为专项验证场景<br>
> 文档版本：2026-09-06（多癌种定位与评测结果修订版）
> 结果口径：在线公开数据源验证、千问全链路验证、规则开发复测、单因素模型对照和公开检索对照分别报告，不将不同运行条件下的数值混作同一成绩。

## 评委故事闭环：问题—设计—指标—能力—价值

本项目的汇报主线不是“我们有多少个 Agent”，而是“一个真实科研问题如何被转化为一份可信数据”。精准治疗研究需要同时找到目标人群、分子变异、治疗方案和疗效记录，但信息分散在论文和多个数据库中，字段写法不同，患者编号只在所属研究中有效。若直接拼接搜索结果，最容易出现错配、无来源发布和跨研究误合并。

因此系统采用有边界的混合式多 Agent：主 Agent 负责统筹，规划 Agent 明确研究条件，检索 Agent 查找多类真实来源，规则模块统一字段并登记 `source_id/raw_field/raw_value`，Critic Agent 独立找缺口和冲突，Quality Agent 结合冻结医学规则决定通过、复核或阻断。这个设计直接对应问题中的四个风险：多来源分散由检索协作解决，字段不一致由规则整理解决，身份和证据不足由 Critic 解决，医学高风险语义由 Quality Gate 解决。

代表任务完成 2 轮、6/6 次真实工具调用，登记 50 条来源记录，形成 1 张主数据表和 3 张补充数据表；主数据表为 115 行 × 31 列、42 名患者，GSE76360 补充表包含 50 名治疗前已有记录的 HER2 阳性患者，其中 48 条疗效记录可核对。严格千问在线全链路 11/11 题实际调用千问、确定性兜底为 0，Retrieval Precision 100.00%、Recall 83.33%、F1 90.91%，Faithfulness 与 Traceability 均为 100.00%，Error F1 与 Repair Accuracy 均为 100.00%，SDTI 为 98.1118；同时保留 3 个遗漏、8 个 REVIEW 任务和 `publish_allowed=false`。

这些指标分别说明具体能力，而不是笼统的“模型准确率”：问题解析 F1 0.5522（较词典方法 +0.0860）说明研究条件边界更清楚；字段匹配 F1 0.9018（较项目原方法 +0.1024）说明不同数据库列名更容易对齐；错误清洗 F1 0.9169 说明有来源依据的错误可以被发现和修复；实体匹配 F1 0.7449 则说明通用算法只能提供候选，患者身份仍必须依靠研究编号、来源和复核。由此形成闭环：问题被拆清，后续查找更聚焦；字段和来源可回查，统计分母更可信；缺口被显式暴露，系统能继续补查而不是编造；医学安全门保留 REVIEW，研究者知道哪些结果可以用、哪些结果还不能发布。

完整讲解稿见：[REVIEWER_STORY.md](REVIEWER_STORY.md)。

## Agent 系统定性与设计取舍

本系统是**有边界的混合式多 Agent 编排系统**，不是单一大 Prompt，也不是无总控的 Agent 集群。`ResearchAgentService` 作为任务级主 Agent，组织问题/研究规划 Agent、Collection Agent、Critic Agent、Quality Agent 和闭环控制器；GDC、GEO、cBioPortal、AACT、CIViC、DepMap Adapter，Schema/Entity 对齐和冻结医学规则属于独立确定性模块。规划只能提出计划，Adapter 只能提供真实来源，Critic 只能诊断，Quality Gate 只能裁决，任何单一角色都不能同时生成事实并宣布发布。

这里的“智能体”不是聊天窗口的换名，而是一个有目标、能选择下一步行动、能调用资料并读取反馈的工作角色。系统设置一个任务总负责人智能体，再把研究规划、资料查找、独立质疑和质量把关交给不同专业智能体；真实数据获取、字段统一、身份核对和医学安全规则由固定模块执行。这样既利用模型的理解和规划能力，又把患者级事实锁在可复核的规则和来源之内。

系统按 `task_id`、闭环轮次、`study_id/patient_id/sample_id` 和来源命名空间隔离上下文，并将模型建议与患者事实分开。独立的只读来源查询、候选检索和健康检查可以并行；共享数据集写入、实体合并、闭环反馈和医学安全裁决必须串行汇合。当前生产主链使用受控串行工具执行，保留并行工具意图，优先保证来源登记、执行顺序和审计可复现。Adapter/Schema、Critic、Quality/Medical Gate 构成独立校验链，模型不能用自评替代校验。

本系统刻意不在单来源元数据查询、格式转换、确定性去重、严格离线基线和低延迟小请求中引入多 Agent，因为这些场景的主要目标是确定性、低延迟和可复现。把所有职能塞进一个 Agent 会造成上下文污染、权限混杂、失败难隔离、无法独立质疑和评测无法归因。具体边界如下。

### 编排边界的具体说明

**职责隔离。** 规划提出 `ResearchSpec` 和工具计划；采集按缺口更换方法；`Critic` 只做诊断；`Quality Gate` 只做裁决；闭环控制器只生成下一轮合法动作。

**上下文隔离。** 系统按 `task_id`、闭环轮次、`study_id/patient_id/sample_id` 和来源命名空间隔离上下文。模型建议不能直接写入患者事实，不同研究的同名编号也不会自动 Join。

**可并行边界。** 独立的只读数据库查询、候选检索和健康检查可以并行表达；共享数据集写入、实体合并、闭环决策和医学裁决必须串行汇合。当前生产主链采用受控串行执行，以保证审计可复现。

**独立校验链。** `Adapter/Schema` 校验先检查响应结构和来源记录，`Critic` 再诊断研究合同与证据缺口，最后由 `Quality Agent` 和冻结医学规则决定准入。模型不能用自己的摘要或自评替代独立校验。

**不该上多 Agent。** 单来源元数据查询、格式转换、确定性去重、严格离线基线和低延迟小请求，使用单模块更快、更便宜，也更容易复现。

**为什么不做单 Agent 多职能。** 单角色同时规划、取数、改数据和自我验收，容易造成上下文污染、权限混杂、无法独立质疑、失败难隔离、并行边界不清和评测无法归因。

### 面向汇报的中文工作流图

![中文智能体协作工作流](../frontend/agent-workflow-cn.svg)

图中用“一个总负责人、多个专业角色”说明系统如何从研究问题走到结果交付：纸张形状表示输入/输出，圆角框表示 Agent，直角框表示固定规则模块，菱形表示判断。独立资料查找可以并行，数据整理、身份对应、质量把关和缺口补查按顺序汇合；简单查询和格式处理则保留为单一确定性步骤。

## 参赛作品简介（300 字内）

多癌种精准治疗研究需要联合患者、基因变异、治疗和疗效数据，但资料分散在论文及多个公开数据库中，字段写法和患者编号也不统一。本系统面向这类科研数据的查找、解析、整合与核验，当前以乳腺癌作为专项验证场景。系统先把自然语言问题整理成数据需求清单，再从真实来源取得记录并按研究分别建表，为每行记录登记来源，并在已接入原始说明的来源上同时展示中文标准值和原始值。乳腺癌代表任务完成 2 轮、6 次真实工具调用，登记 50 条来源记录，形成 1 张主数据表和 3 张补充数据表；在 GSE76360 研究中得到 50 名治疗前已有记录的患者和 48 条有效疗效记录。系统最终交付数据表、字段说明、来源清单和质量说明，使数据来源、处理依据、适用范围及后续补查条件均可核查。

## 摘要

方向 1A 要求参赛作品把科研人员提出的问题转化为可分析、可追溯、可复用的数据。本文以乳腺癌专项验证中的“研究 HER2 阳性乳腺癌中 PIK3CA 突变与手术前治疗效果（下文简称‘疗效’）的关系”为例，说明系统如何处理多癌种科研数据整合中具有代表性的难题。真正需要交付的不是一段医学知识，而是一份同时包含目标人群、基因变异、治疗、疗效和患者身份的数据表。查找论文和数据库登记信息后发现，GSE76360 研究提供 HER2 阳性患者的术前疗效，METABRIC 研究提供 PIK3CA 与临床资料，阿培利司（一种乳腺癌靶向药）治疗研究同时提供 PIK3CA 与疗效。虽然这些来源能够相互补充，但它们来自不同研究中的患者组（医学研究中称为“队列”），患者编号和疗效定义并不通用，因此不能只凭主题相近直接拼接。
...
**关键词：** 人工智能驱动科学研究；科学数据整合；多癌种精准治疗；乳腺癌专项验证；患者与样本关联；数据来源追踪
...
多癌种精准治疗研究并不是“问模型一个问题、得到一段答案”这么简单。研究者往往需要同时找到目标人群、分子变异、治疗阶段和疗效记录，而这些内容分散在论文、公共数据库和不同格式的附件中。同一个字段可能有多种写法，同一个患者也可能有多份样本；如果只按关键词拼接，容易把不同研究中的患者误认为同一人，也容易把模型生成的解释误当成真实数据。乳腺癌专项验证把这些共性问题具体化为 HER2、PIK3CA、治疗阶段和疗效的联合核对。
...
| 科学检索 | BEIR 五任务、3,677 条查询；前十条排序、前十命中、前一百条召回 | **BM25+BGE 名次融合：0.3915 / 0.7049 / 0.6677** | 公开 BGE 单路：0.3880 / 0.6946 / 0.6554 | **+0.0035 / +0.0102 / +0.0123**；排序、命中和覆盖同时改善 |
...
这些结果同时说明了作品的优势边界：字段语义改名、特定来源约束和表内可验证错误可以明显改善；通用复杂检索、干预 span 边界和缺少身份线索的实体匹配仍需要专门模型或人工复核。乳腺癌专项任务中的千问在线链路也单独记录，不与公开模块分数合并。
...
| 检索前十条排序 **0.3915** | 相关论文和数据说明是否排在前面 | 比 BGE 单路提高 **0.0035**；前百召回同时提高 **0.0123** | 研究者更快看到有用材料，并扩大两路检索共同覆盖；两路都遗漏的材料仍需补查 |
...
![图 1 当前科研规划工作台（重新采集的干净界面）](images/frontend-home-clean-20260906.png)

*图 1 为 2026-09-06 重新采集的当前入口原图。左侧五阶段导航负责推进研究，中央区域提供跨癌种示例问题与一句话输入，右侧标签页承载研究依据、研究方案、数据准备和覆盖矩阵。*

从图 1 可以看出，系统先把宽泛研究想法组织为可推进的五阶段工作，再进入真实规划、来源查找和数据准备。页面上的示例卡片只负责帮助用户形成问题，不能替代后端返回的来源、字段和质量证据。乳腺癌专项验证仍沿用后文所述的代表任务统计；当前入口截图与历史运行结果分开记录，避免把界面状态误读为数据成绩。
...
![图 7（历史代表任务）GSE76360 患者—样本记录与有效疗效数](images/annotated-positive-06-gse76360-table-20260831.png)
...
![图 8（历史代表任务）样本 GSM1982110 的中文标准值与 GEO 原值核对](images/annotated-positive-07-raw-audit-20260831.png)
...
![图 9（历史代表任务）代表任务的四层质量判定与主数据表](images/annotated-positive-05-quality-gate-20260831.png)
...
规划模型决定“要查什么”，检索方法决定“哪些材料先被找到”。为了判断关键词匹配和语义理解的作用，本文在 5 个公开数据集、3,677 条查询上比较公开基线与本项目方法：关键词检索 BM25、语义检索 BGE、两者的历史分数组合，以及本项目当前采用的名次融合。交叉编码器重排另在 SciFact 的 300 个测试问题上单独验证。

![图 11 三种公开检索方法的各数据集平均结果](images/retrieval-method-comparison-cn-20260831.png)

*图 11 先在每个公开数据集分别计算再取平均后，名次融合在前十条排序质量和前 100 条召回率上高于单路 BGE；SciDocs 等任务仍保留纯语义方法的优势。*
...
| **本项目当前方法** | **BM25+BGE 名次融合** | **0.3915** | **0.6677** | **0.4430** | 8.99 毫秒（批次均摊） |

由图 11 和表 10 可知，BM25 和 BGE 是公开基线，BM25+BGE 分数融合是本项目早期组合；当前方法改用名次融合，以避免两路分数尺度不同造成偏置。与公开 BGE 基线相比，当前方法的前十条排序质量由 `0.3880` 提升到 `0.3915`，前一百条召回率由 `0.6554` 提升到 `0.6677`，首个相关结果排序得分由 `0.4421` 提升到 `0.4430`。五任务平均速度为 `8.99` 毫秒（批次均摊），仍接近 BGE 单路。系统因此把关键词检索与语义检索组合在同一条候选发现链上，再由研究条件清单与来源规则核对；需要更精细前排排序时，再启用 SciFact 已验证的交叉重排。
...
各模块逐任务图、对照方法、指标定义和结果边界见[公开数据集统一对照报告](../evaluation/PUBLIC_DATASET_COMPARISON_20260902.md)。本补充图组只扩展公开对照说明，不修改前文已有的系统架构图、流程图和内容结构图。
...
| 科学检索 | BEIR 五任务、3,677 条查询；前十条排序质量 | **本项目 BM25+BGE 名次融合 / 0.3915** | 公开 BGE 单路 / 0.3880 | **+0.0035** |
...
| 科学检索 | [BEIR 官方仓库](https://github.com/beir-cellar/beir)；[数据集列表](https://github.com/beir-cellar/beir/wiki/Datasets-available)；本轮共同完成 [SciFact](https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/scifact.zip)、[NFCorpus](https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/nfcorpus.zip)、[SciDocs](https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/scidocs.zip)、[ArguAna](https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/arguana.zip) 和 [FiQA](https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/fiqa.zip)；另补测 [Quora](https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/quora.zip) 与 [TREC-COVID](https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/trec-covid.zip) | [BM25 官方评测示例](https://github.com/beir-cellar/beir/blob/main/examples/retrieval/evaluation/lexical/evaluate_bm25.py)；[BGE 官方实现](https://github.com/FlagOpen/FlagEmbedding)；重排器 [ms-marco-MiniLM-L6-v2](https://huggingface.co/cross-encoder/ms-marco-MiniLM-L6-v2) | **BM25+BGE 名次融合**：在开发标注上选择两路权重和融合常数；BM25 调参、分数融合、哈希混合和 SciFact 交叉重排是消融版本 |
...
来源索引表说明了每组结果的可复现入口。BEIR 五项任务中，SciFact、NFCorpus、SciDocs、ArguAna 和 FiQA 用于五种基础方法的共同宏平均；Quora 已完成 BM25 与调参 BM25 的公开测试，但语义方法尚未形成完整运行，因此只列分层结果；TREC-COVID 是额外的医学文献补充任务。Valentine、DeepMatcher 和 Raha/HoloClean 分别提供字段匹配、实体匹配和错误清洗的公开测试数据。项目历史版本和当前版本都在同一测试任务上评分，公开基线也使用同一测试集和同一评价规则，因此表内数字可以横向比较；不同能力之间不合并为一个“总准确率”。

本项目各方法的实现和运行记录统一保存在[项目公开仓库](https://github.com/xsc2466729313-cyber/research-data-agent)；论文中的结果表只展示已完成运行的真实结果，未完成的公开任务只保留来源链接和状态说明。
...
#### 3.11.3 五个共同公开数据集的多方法统一对照

为了让结果能够直接比较，下表汇总五种基础方法均已完整运行的五个公开数据集，共 `3,677` 个测试问题。其中，名称前带“公开基线”的是通用对照方法，名称前带“本项目”的是本系统实现或组合的方法。前十条排序质量衡量相关材料是否被排在前面；命中率衡量前若干条中是否至少出现一篇相关材料；召回率衡量全部相关材料被找回的比例；首条排序越高，说明研究者越早看到第一篇相关材料。表内结果均先在各数据集内计算，再对五个数据集等权平均。

| 方法身份 | 数据集 / 问题数 | 前十条排序质量 | 命中@1 | 命中@3 | 命中@5 | 命中@10 | 召回@10 | 召回@100 | 首条排序 | 平均耗时 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 公开基线：BM25 | 5 / 3,677 | 0.3139 | 0.2574 | 0.4294 | 0.5027 | 0.5980 | 0.3887 | 0.5554 | 0.3615 | 50.49 毫秒 |
| 本项目：调参 BM25 | 5 / 3,677 | 0.3147 | 0.2574 | 0.4313 | 0.5065 | 0.6011 | 0.3902 | 0.5552 | 0.3629 | 50.41 毫秒 |
| 本项目消融：哈希混合检索 | 5 / 3,677 | 0.1834 | 0.1815 | 0.2697 | 0.3116 | 0.3629 | 0.1978 | 0.3355 | 0.2359 | 85.64 毫秒 |
| **公开基线：BGE 语义检索** | **5 / 3,677** | **0.3880** | **0.3201** | **0.5255** | **0.6072** | **0.6946** | **0.4663** | **0.6554** | **0.4421** | **7.67 毫秒（批次均摊）** |
| 本项目：BM25+BGE 分数融合 | 5 / 3,677 | 0.3791 | 0.3124 | 0.5019 | 0.5867 | 0.6813 | 0.4619 | 0.6422 | 0.4277 | 54.05 毫秒（批次均摊） |
| **本项目：BM25+BGE 名次融合** | **5 / 3,677** | **0.3915** | **0.3179** | **0.5292** | **0.6103** | **0.7049** | **0.4776** | **0.6677** | **0.4430** | **8.99 毫秒（批次均摊）** |

这张总表给出了三个清楚的结论。第一，名次融合的宏平均 nDCG@10 为 `0.3915`，比公开 BGE 高 `0.0035`；前十命中率提高 `0.0102`，前百召回率提高 `0.0123`，说明关键词和语义两路的互补候选能够同时改善前排可见性和候选覆盖。第二，原来的分数融合为 `0.3791`，低于 BGE，说明不同模型的原始分数不能直接相加，名次融合解决了分数尺度不一致的问题。第三，哈希混合在五项任务上均明显落后并增加耗时，因此保留为失败消融；调参 BM25 只带来轻微改善，适合作为资源受限时的低成本方案。

#### 3.11.4 分数据集观察：优势出现在哪里

宏平均会掩盖任务差异，因此下表把同一批基础方法放回各自数据集。每一行使用该数据集完整测试问题，五个分数可以横向比较；“本行最优”明确标出该任务上表现最好的方法。

| 公开数据集（点击名称查看原始数据） | 测试问题数 | 公开 BM25 | 本项目调参 BM25 | 本项目哈希混合 | 公开 BGE | 本项目名次融合 | 本行最优 |
|---|---:|---:|---:|---:|---:|---:|---|
| [ArguAna（论证检索）](https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/arguana.zip) | 1,406 | 0.3067 | 0.3067 | 0.0708 | 0.3836 | **0.3852** | 本项目名次融合 |
| [NFCorpus（营养医学）](https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/nfcorpus.zip) | 323 | 0.2899 | 0.2902 | 0.2493 | 0.3315 | **0.3427** | 本项目名次融合 |
| [SciDocs（科学文献关系）](https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/scidocs.zip) | 1,000 | 0.1490 | 0.1490 | 0.0907 | **0.1910** | 0.1828 | 公开 BGE |
| [SciFact（科学事实）](https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/scifact.zip) | 300 | 0.6040 | 0.6044 | 0.4070 | 0.6803 | **0.6829** | 本项目名次融合 |
| [FiQA（金融问答检索）](https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/fiqa.zip) | 648 | 0.2197 | 0.2230 | 0.0992 | 0.3533 | **0.3637** | 本项目名次融合 |
| [Quora（问题检索）](https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/quora.zip) | 10,000 | 0.7393 | **0.7408** | 未完成 | 未完成 | 未完成 | 本项目调参 BM25 |

表中数值均为前十条排序质量（nDCG@10），不是准确率。名次融合在 ArguAna、NFCorpus、SciFact 和 FiQA 上超过公开 BGE，在 SciDocs 上低于公开 BGE；Quora 当前只完成 BM25 两种方法，未完成方法不填数。结果说明名次融合能够改善多数本轮任务的前排排序，但论文关系检索仍应优先使用纯语义方法；不同公开任务必须保留分层结论，不能用一个宏平均掩盖单项短板。
...
将开发集参数选择用于 BM25+BGE 名次融合后，本项目在 SciFact、NFCorpus、SciDocs、ArguAna 和 FiQA 五项任务上的前十条排序质量为 `0.3915`，公开 BGE 单路为 `0.3880`；前十命中率由 `0.6946` 提升到 `0.7049`，前百召回率由 `0.6554` 提升到 `0.6677`。这一结果说明名次融合的收益主要来自两路候选的互补覆盖，而不是改变测试集或逐题选择答案。交叉重排仍只在 SciFact 的 `300` 个测试问题上形成完整消融，前十条排序质量为 `0.6872`，用于说明精细重排对前排位置的额外收益及其速度代价。
...
当前根地址已恢复为“新研究 / 提出方向”规划工作台。以下截图均为 2026-09-06 重新采集：干净原图确认布局，红色框、箭头和中文说明版把解释移到画布外，不遮挡界面；后续结果图应按 [前端与内核截图索引](FRONTEND_SCREENSHOT_INDEX.md) 的 R 组区分真实运行、REVIEW 运行和历史代表任务。

![图 2 当前科研规划工作台桌面布局（干净原图）](images/frontend-home-clean-20260906.png)

*图 2 展示五阶段导航、示例问题卡、研究问题输入区和右侧四类审查标签；对应红色讲解版见 `images/frontend-home-annotated-20260906.png`。*

![图 3 当前科研规划工作台移动端布局（干净原图）](images/frontend-home-mobile-clean-20260906.png)

*图 3 展示窄屏下示例卡片、输入区和审查标签的顺序重排；页面仍保留“发送并开始研究”入口。红色框/箭头/文字讲解版见 `images/frontend-home-mobile-annotated-20260906.png`。*

### 4.1 科研助手、内核实验室与 Agent 流程图

科研助手（科研兔）是独立的只读解析入口，不与研究问题输入区混淆：

![科研助手（科研兔）红色讲解标注](images/research-companion-panel-annotated-20260906.png)

内核实验室前端展示研究协议、配置边界、Agent 架构、Runtime 和一句话任务入口。空闲壳层只能说明页面结构；`loop-91ef39cffd32:r3` 的真实运行证据、7/7 Runtime 和四层 PASS 见截图索引 R 组。

![内核实验室前端红色讲解标注](images/kernel-lab-desktop-annotated-20260906.png)

![Agent 架构流程图红色讲解标注](images/kernel-lab-agent-architecture-annotated-20260906.png)

当前版本按参考样式补充两张结果审查讲解图：

![质量门与结构化数据红框讲解图](images/kernel-lab-current-quality-data-callout-20260906.png)

![原始样本特征弹窗红框讲解图](images/kernel-lab-current-raw-characteristics-callout-20260906.png)

图中红色框、箭头和文字用于把质量门分组、结构化患者/样本计数及原始值审计位置指给读者；它们来自同一真实运行快照，不新增或替换评测分数。

### 4.1 研究条件核对

研究用途判断的前提是任务条件理解正确。因此，页面首先呈现研究人群、分子变量、治疗阶段、疗效和必需字段，并将这些条件与当前数据矩阵并列展示。结合历史代表任务的图 7—图 14 可知，115 行、31 列不是脱离任务口径的数据规模，而是由已冻结研究条件限定的结构化结果。
...
![图 13（历史代表任务）来源规模统计与代表性发现—选择路径](images/annotated-positive-08-lineage-20260831.png)
...
![图 14（历史代表任务）主数据表的研究可用性与字段字典](images/annotated-positive-09-readiness-20260831.png)
...
本文围绕方向 1A 的科学数据场景，研究了自然语言科研问题如何转化为可分析、可追溯、可复用的数据包。多癌种精准治疗数据的共同难点不在于来源数量，而在于目标变量分散、相近字段医学含义不同、患者编号只在所属研究中有效。乳腺癌专项验证将这些问题落实为可检查的代表任务。基于这些事实，本文把问题理解、文献和来源发现、真实取数、按研究建表、字段整理、身份核对、四层质量检查和结果导出组织为一条连续数据链。
...
2. [公开数据集分层指标报告](../evaluation/PUBLIC_DATASET_COMPARISON_20260902.md)
3. [系统设计报告](PROJECT_REPORT.md)
...
8. [公开数据集统一对照报告](../evaluation/PUBLIC_DATASET_COMPARISON_20260902.md)
9. [公开检索的各数据集平均结果](../evaluation/PUBLIC_RETRIEVAL_MATRIX_20260903.md)
...
15. 在线公开数据源健康检查脚本：仓库 `scripts/check_live_source_health.py`
16. [2026-09-01 在线公开数据源检查结果](../evaluation/source_health_20260901.json)
17. [2026-09-02 在线数据链开发复测指标](../goldset/breast_cancer/official_candidate/evaluation_runs/official-candidate-current-deterministic-baseline-20260902/metrics.json)
18. [2026-09-02 在线数据链开发复测审计记录](../goldset/breast_cancer/official_candidate/evaluation_runs/official-candidate-current-deterministic-baseline-20260902/AUDIT.json)
