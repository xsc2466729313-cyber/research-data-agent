...
| 正式指标观察 | 历史基线 **63.36**；严格 Qwen LIVE 候选 **97.53**，召回率 **83.33%**；两者均 `publish_allowed=false` 且不是 `frozen_test` | BEIR / Valentine / DeepMatcher；development 千问 LIVE **66.94（非正式）** | — |
...
- 最新严格千问产物：`goldset/breast_cancer/official_candidate/evaluation_runs/official-candidate-qwen-live-recall-v9-20260902/`（`metrics.json` / `report.md` / `AUDIT.json`）
...
| 严格 Qwen LIVE 候选 | **97.53**，publish_allowed=false | 11/11 Qwen；召回率 83.33%；8 个任务 REVIEW；不是 frozen_test |
...
正式 SDTI 入口是 `goldset/templates/`。当前严格 Qwen LIVE 观察产物为 `official-candidate-qwen-live-recall-v9-20260902`（97.53）；11/11 题实际调用千问、确定性兜底 0 次，运行时来源校验 259/259 通过，但 8 个任务质量门 REVIEW 且卷面未 sealed，不得自动发布。`goldset/breast_cancer/development/` 的 66.94 仅作非正式对照。
