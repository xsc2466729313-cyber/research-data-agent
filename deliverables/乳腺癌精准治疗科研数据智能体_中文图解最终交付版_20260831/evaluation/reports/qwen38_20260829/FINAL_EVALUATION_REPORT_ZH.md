...
- `query_understanding_ablation.json`：75 条查询的 A—E 总体、数据集和长度分层结果。
- `query_understanding_full_ab.json`：3,677 条查询的全量 A/B 结果。
- `planner_replacement_ablation.json`：千问与 DeepSeek 主 18 次运行结果。
- `report_metrics_summary.json`：总体、分层、查询表达、模型替换和闭环摘要。
- `../../vnext_retrieval_calibrated_macro_20260828.json`：关键词检索、语义检索和简单组合的五数据集结果。

本压缩包面向论文阅读与数字核对，不包含完整项目源码、模型会话缓存或接口密钥；正文所使用的对照数字均可在上述随包文件中核对。
