...
`official_candidate/` 是 held-out 正式考卷。独立审核人 **xsc** 于 2026-08-29 审核通过并写入 `goldset/templates/`。题面与金标准行未整份复制 development。`frozen=false`，不是 `frozen_test`。正式评测路径：`POST /api/evaluation/official-run` 或 `collect_official_sdti.py`。审核记录见 `official_candidate/AUDIT.md`。数字对照见 `docs/DATA_REPORT_20260829.md`。
