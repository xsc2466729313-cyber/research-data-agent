...
- 项目：cancer-precision-data-agent（仓库显示名：肿瘤精准治疗科研数据智能整合系统）
...
3. `docs/06_评测指标与SDTI.md`：评测公式。改公式等于改比赛口径。
...
下一步不应重构，而应在不触碰冻结内核的前提下，增量增加 Router、Scientific Planner、Data Discovery、Multimodal Extraction 和 Evidence Graph。这样，系统才能从“乳腺癌精准治疗科研数据智能体”，成长为“通用科研数据发现与可信整合智能体”，同时保住现在真正值钱的东西。
